...
if (url_id("dept")) {
	...
	$departmentID = ($user["isHelpdesk"]) ? $user["departmentID"] : 8;
}
...
if (isset($_GET["ticketID"])) {
	if (isset($_GET["newOwner"])) {
        ...
        db_query("UPDATE helpdesk_tickets SET ownerID = {$_GET["newOwner"]}, statusID = {$r}, updatedOn  = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
        ...
        db_query("UPDATE helpdesk_tickets SET statusID = {$_GET["newStatus"]}, updatedOn  = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
	} elseif  (isset($_GET["newPriority"])) {
        db_query("UPDATE helpdesk_tickets SET priorityID = {$_GET["newPriority"]}, updatedOn  = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
		//email mohammed for critical
	} elseif (isset($_GET["newDepartment"])) {
		db_query("UPDATE helpdesk_tickets SET departmentID = {$_GET["newDepartment"]}, updatedOn = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
	} elseif (isset($_GET["newType"])) {
		if (empty($_GET["newType"])) $_GET["newType"] = "NULL";
		db_query("UPDATE helpdesk_tickets SET typeID = {$_GET["newType"]}, updatedOn = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
	} elseif (isset($_GET["newTime"])) {
		if (empty($_GET["newTime"])) $_GET["newTime"] = 0;
		db_query("UPDATE helpdesk_tickets SET timeSpent = {$_GET["newTime"]}, updatedOn = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
	} elseif (isset($_GET["newDepartment"])) {
		db_query("UPDATE helpdesk_tickets SET departmentID = {$_GET["newDepartment"]}, typeID = NULL, updatedOn = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
	} elseif (isset($_GET["newUser"])) {
		db_query("UPDATE helpdesk_tickets SET createdBy = {$_GET["newUser"]}, updatedOn = GETDATE(), updatedBy = {$user["id"]} WHERE id = " . $_GET["ticketID"]);
	}
...
function emailITticket($id, $scenario, $admin=false) {
	...
	if ($scenario == "followup") {
		...
		$message .= drawServerMessage("<a href='http://" . $_josh["request"]["host"] . "/staff/view.php?id=" . $user["id"] . "'>" . $user["full_name"] . "</a> just made an administrative followup on this Helpdesk ticket.  Regular staff were not copied on this message.");
		...
		$message .= drawServerMessage("<a href='http://" . $_josh["request"]["host"] . "/staff/view.php?id=" . $user["id"] . "'>" . $user["full_name"] . "</a> has assigned this ticket to <a href='http://" . $_josh["request"]["host"] . "/staff/view.php?id=" . $ticket["ownerID"] . "'>" . $ticket["ownerName"] . "</a>.  <b>Don't reply to this email!</b>  Instead, please <a href='http://" . $_josh["request"]["host"] . "/helpdesk/ticket.php?id=" . $id . "'>view your ticket</a> in the intranet ticketing system.<br><br><b>Note:</b> if you add this sender to your 'safe senders list,' pictures will always download.");
	} elseif ($scenario == "new") {
	...
	$admins = arrayRemove($user["email"], $admins);
	
	$users = array_unique($users);
	$users = arrayRemove($user["email"], $users);
