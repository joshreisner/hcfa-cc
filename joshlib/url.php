...
function url_change($redirect="") {
	...
	if ($redirect === false) { //if redirect is really set to FALSE, send to site home
		$redirect = "/";
	} elseif (empty($redirect)) { //if redirect is an empty string, refresh the page by sending back to same URL
		$redirect = $_josh["request"]["path_query"];
	}
	if ($_josh["slow"]) {
		error_debug("<b>url_change</b> (slow) to " . $redirect);
		if ($_josh["debug"]) db_close();
		echo "<html><head>
...
function url_change_post($secondaryTarget) {
	global $_POST;
	if (!isset($_POST["return_to"]) || empty($_POST["return_to"])) $_POST["return_to"] = $secondaryTarget;
...
function url_query_add($adds, $go=true) { //add specified query arguments
	global $_josh, $_GET;
